...
(0, node_test_1.describe)('InterruptibleEntity', async () => {
    ...
    (0, node_test_1.test)('basic', async (t) => {
        ...
        for (const op of ['load']) {
            if ((0, utility_1.maybeSkipControl)(t, 'entityOp', 'interruptible.' + op, live))
                return;
        }
        ...
        let interruptible_ref01_data = Object.values(setup.data.existing.interruptible)[0];
        // LOAD
        const interruptible_ref01_ent = client.Interruptible();
        const interruptible_ref01_match_dt0 = {};
        const interruptible_ref01_data_dt0 = (await interruptible_ref01_ent.load(interruptible_ref01_match_dt0)).data();
        (0, node_assert_1.default)(null != interruptible_ref01_data_dt0);
    });
});
function basicSetup(extra) {
    ...
    const entityDataFile = node_path_1.default.resolve(__dirname, '../../../../.sdk/test/entity/interruptible/InterruptibleTestData.json');
    ...
    let idmap = transform(['interruptible01', 'interruptible02', 'interruptible03'], {
        '`$PACK`': ['', {
                '`$KEY`': '`$COPY`',
    ...
    const idmapEnvVal = process.env['ELERING_DASHBOARD_TEST_INTERRUPTIBLE_ENTID'];
...
//# sourceMappingURL=InterruptibleEntity.test.js.map