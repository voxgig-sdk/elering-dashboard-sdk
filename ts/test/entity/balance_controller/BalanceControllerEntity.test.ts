...
describe('BalanceControllerEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'balance_controller.' + op, live)) return
    }
    ...
    const balance_controller_ref01_data_dt0 = await balance_controller_ref01_ent.load(balance_controller_ref01_match_dt0)
