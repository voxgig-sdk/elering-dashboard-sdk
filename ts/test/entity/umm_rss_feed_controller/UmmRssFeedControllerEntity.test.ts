...
describe('UmmRssFeedControllerEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'umm_rss_feed_controller.' + op, live)) return
    }
    ...
    const umm_rss_feed_controller_ref01_data_dt0 = await umm_rss_feed_controller_ref01_ent.load(umm_rss_feed_controller_ref01_match_dt0)
