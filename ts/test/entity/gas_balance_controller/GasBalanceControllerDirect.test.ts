...
describe('GasBalanceControllerDirect', async () => {
  ...
  test('direct-load-gas_balance_controller', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-gas_balance_controller', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['ELERING_DASHBOARD_TEST_GAS_BALANCE_CONTROLLER_ENTID']
