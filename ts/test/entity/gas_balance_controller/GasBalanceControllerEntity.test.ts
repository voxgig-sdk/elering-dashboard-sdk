...
describe('GasBalanceControllerEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'gas_balance_controller.' + op, live)) return
    }
    ...
    const gas_balance_controller_ref01_data_dt0 = await gas_balance_controller_ref01_ent.load(gas_balance_controller_ref01_match_dt0)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['ELERING_DASHBOARD_TEST_GAS_BALANCE_CONTROLLER_ENTID']
