...
describe('GreenControllerDirect', async () => {
  ...
  test('direct-load-green_controller', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-green_controller', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['ELERING_DASHBOARD_TEST_GREEN_CONTROLLER_ENTID']
