...
describe('InterruptibleCapacityControllerDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new EleringDashboardSDK({
  ...
  test('direct-load-interruptible_capacity_controller', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-interruptible_capacity_controller', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['ELERING_DASHBOARD_TEST_INTERRUPTIBLE_CAPACITY_CONTROLLER_ENTID']
