...
describe('FirmCapacityControllerDirect', async () => {
  ...
  test('direct-load-firm_capacity_controller', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-firm_capacity_controller', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['ELERING_DASHBOARD_TEST_FIRM_CAPACITY_CONTROLLER_ENTID']
