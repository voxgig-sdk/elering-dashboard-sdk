...
describe('GasTransmissionControllerDirect', async () => {
  ...
  test('direct-load-gas_transmission_controller', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-gas_transmission_controller', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['ELERING_DASHBOARD_TEST_GAS_TRANSMISSION_CONTROLLER_ENTID']
