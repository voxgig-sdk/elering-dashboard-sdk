...
describe('UmmGasControllerEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'umm_gas_controller.' + op, live)) return
    }
    ...
    const umm_gas_controller_ref01_data_dt0 = await umm_gas_controller_ref01_ent.load(umm_gas_controller_ref01_match_dt0)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['ELERING_DASHBOARD_TEST_UMM_GAS_CONTROLLER_ENTID']
