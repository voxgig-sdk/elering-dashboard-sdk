...
boru-driven command-line client **and** interactive REPL for the EleringDashboard
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/elering-dashboard-cli)
make build

# 2. See usage (words, entities, env vars)
./elering-dashboard-cli --help

# 3. Provide credentials once, via the environment
export ELERING_DASHBOARD_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./elering-dashboard-cli load 1 balance            # {id:1} shorthand
./elering-dashboard-cli load '{id:1}' balance       # explicit match map

# 5. Override the API base URL for a single call
ELERING_DASHBOARD_BASE=https://api.example.com ./elering-dashboard-cli load 1 balance

# 6. No arguments -> interactive REPL
./elering-dashboard-cli
elering-dashboard> load 1 balance
elering-dashboard> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/elering-dashboard-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/elering-dashboard-cli load 1 balance
   ```
...
```sh
./elering-dashboard-cli load 1 balance          # scalar shorthand for {id:1}
./elering-dashboard-cli load '{id:1}' balance     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `elering-dashboard>`). Each line is
evaluated as its own boru expression:

```text
$ ./elering-dashboard-cli
elering-dashboard> load 1 balance
elering-dashboard> /help
elering-dashboard> /quit
```
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/elering-dashboard-cli`. |
...
(`./elering-dashboard-cli <expr>`) and interactively (the REPL), and expressions compose the
