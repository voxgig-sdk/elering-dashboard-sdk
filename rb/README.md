...
The Ruby SDK for the EleringDashboard API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
result, err = client.Balance(nil).load({ "id" => "example_id" }, nil)
raise err if err
puts result
```
...
result, err = client.EleringDashboard(nil).load(
  { "id" => "test01" }, nil
)
