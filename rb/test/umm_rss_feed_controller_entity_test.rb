...
class UmmRssFeedControllerEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = umm_rss_feed_controller_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "umm_rss_feed_controller." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set ELERING_DASHBOARD_TEST_UMM_RSS_FEED_CONTROLLER_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    umm_rss_feed_controller_ref01_data = nil
    if umm_rss_feed_controller_ref01_data_raw.length > 0
...
def umm_rss_feed_controller_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "umm_rss_feed_controller", "UmmRssFeedControllerTestData.json")
  ...
  entid_env_raw = ENV["ELERING_DASHBOARD_TEST_UMM_RSS_FEED_CONTROLLER_ENTID"]
