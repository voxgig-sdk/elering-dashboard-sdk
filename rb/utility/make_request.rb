...
module EleringDashboardUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = EleringDashboardResponse.new({})
    ...
    if fetch_err
      ...
      response = EleringDashboardResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = EleringDashboardResponse.new(fetched)
    else
