...
class InterruptibleCapacityControllerEntity
  def initialize(client, entopts = nil)
    ...
    @_name = "interruptible_capacity_controller"
    @_client = client
  ...
  def make
    opts = @_entopts.dup
    InterruptibleCapacityControllerEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = EleringDashboardHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [InterruptibleCapacityController, Hash] the current InterruptibleCapacityController data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = EleringDashboardHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of InterruptibleCapacityController fields)
  def match_get
  ...
  #
  # @param reqmatch [InterruptibleCapacityControllerLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.InterruptibleCapacityController.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [InterruptibleCapacityController, Hash] the loaded InterruptibleCapacityController; raises EleringDashboardError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = EleringDashboardHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
