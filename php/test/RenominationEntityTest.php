...
class RenominationEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = EleringDashboardSDK::test(null, null);
        $ent = $testsdk->Renomination(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = renomination_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "renomination." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        $renomination_ref01_data = null;
        if (count($renomination_ref01_data_raw) > 0) {
...
function renomination_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/renomination/RenominationTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["renomination01", "renomination02", "renomination03"] as $k) {
