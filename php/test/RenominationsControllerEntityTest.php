...
class RenominationsControllerEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = EleringDashboardSDK::test(null, null);
        $ent = $testsdk->RenominationsController(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = renominations_controller_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "renominations_controller." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set ELERING_DASHBOARD_TEST_RENOMINATIONS_CONTROLLER_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $renominations_controller_ref01_data = null;
        if (count($renominations_controller_ref01_data_raw) > 0) {
...
function renominations_controller_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/renominations_controller/RenominationsControllerTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["renominations_controller01", "renominations_controller02", "renominations_controller03"] as $k) {
    ...
    $entid_env_raw = getenv("ELERING_DASHBOARD_TEST_RENOMINATIONS_CONTROLLER_ENTID");
