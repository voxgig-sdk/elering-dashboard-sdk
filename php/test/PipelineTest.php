...
// EleringDashboard SDK pipeline test
//
...
{
    private static function utility(): EleringDashboardUtility
    {
        return new EleringDashboardUtility();
    }
    ...
    private static function ctx(array $over = []): EleringDashboardContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new EleringDashboardContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new EleringDashboardOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof EleringDashboardError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new EleringDashboardOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = EleringDashboardMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new EleringDashboardOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = EleringDashboardMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new EleringDashboardOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = EleringDashboardMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new EleringDashboardSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = EleringDashboardMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new EleringDashboardResponse([]),
            'result' => new EleringDashboardResult([]),
        ]);
        [, $err] = EleringDashboardMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new EleringDashboardSpec([]),
            'response' => null,
            'result' => new EleringDashboardResult([]),
        ]);
        [, $err] = EleringDashboardMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new EleringDashboardSpec(['step' => 's']),
            'response' => new EleringDashboardResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new EleringDashboardResult([]),
        ]);
        [, $err] = EleringDashboardMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new EleringDashboardSpec(['step' => 's']),
            'response' => new EleringDashboardResponse(self::resp(200, ['v' => 1])),
            'result' => new EleringDashboardResult([]),
        ]);
        [, $err] = EleringDashboardMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new EleringDashboardSpec(['step' => 's']),
            'response' => new EleringDashboardResponse(self::resp(200, ['v' => 2])),
            'result' => new EleringDashboardResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        EleringDashboardMakeResponse::call($ctx);
    ...
    {
        $preset = new EleringDashboardResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new EleringDashboardSpec([]),
            'response' => new EleringDashboardResponse([]),
            'result' => new EleringDashboardResult([]),
        ...
        [$got, $err] = EleringDashboardMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new EleringDashboardResult([])]);
        [, $err] = EleringDashboardMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new EleringDashboardSpec([]), 'result' => null]);
        [, $err] = EleringDashboardMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new EleringDashboardSpec(['step' => 's']),
            'result' => new EleringDashboardResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new EleringDashboardOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = EleringDashboardMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new EleringDashboardSpec(['step' => 's']),
            'result' => new EleringDashboardResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new EleringDashboardOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = EleringDashboardMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new EleringDashboardResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new EleringDashboardSpec([]),
            'result' => new EleringDashboardResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = EleringDashboardMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = EleringDashboardMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new EleringDashboardError('boom', 'boom');
        $utility->fetcher = function (EleringDashboardContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new EleringDashboardSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = EleringDashboardMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (EleringDashboardContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = EleringDashboardMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(EleringDashboardResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        EleringDashboardMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (EleringDashboardContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new EleringDashboardSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = EleringDashboardMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new EleringDashboardResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new EleringDashboardSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = EleringDashboardMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = EleringDashboardMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = EleringDashboardMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new EleringDashboardResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, EleringDashboardDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new EleringDashboardResult(['ok' => false])]);
        $this->expectException(EleringDashboardError::class);
        EleringDashboardDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new EleringDashboardResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', EleringDashboardMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        EleringDashboardMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new EleringDashboardBaseFeature();
        $b = new EleringDashboardBaseFeature();
        EleringDashboardFeatureAdd::call($ctx, $a);
        EleringDashboardFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): EleringDashboardBaseFeature
    {
        $f = new EleringDashboardBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        EleringDashboardFeatureAdd::call($ctx, self::named_feature('a'));
        EleringDashboardFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        EleringDashboardFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        EleringDashboardFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        EleringDashboardFeatureAdd::call($ctx, $replace);
        ...
        EleringDashboardFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): EleringDashboardContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new EleringDashboardSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = EleringDashboardPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        EleringDashboardPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new EleringDashboardResponse(['headers' => null]),
            'result' => new EleringDashboardResult([]),
        ]);
        EleringDashboardResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        EleringDashboardResultBody::call($ctx);
