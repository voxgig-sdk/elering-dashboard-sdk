...
// EleringDashboard SDK feature test
//
...
class FtCtrl extends EleringDashboardControl
{
...
class FtClient
{
    ...
    private EleringDashboardUtility $utility;

    public function __construct(EleringDashboardUtility $utility, array $options)
    {
    ...
    public function get_utility(): EleringDashboardUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (EleringDashboardContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public EleringDashboardContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (EleringDashboardContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = EleringDashboardConfig::make_feature($name);
    ...
    private static function buildUrl(EleringDashboardSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof EleringDashboardError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new EleringDashboardResponse($fetched);
        }
    ...
    private function fail(EleringDashboardContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (EleringDashboardContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
