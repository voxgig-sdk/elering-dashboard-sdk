...
// EleringDashboard SDK retry feature
...
class EleringDashboardRetryFeature extends EleringDashboardBaseFeature
{
    ...
    public function init(EleringDashboardContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (EleringDashboardContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(EleringDashboardContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(EleringDashboardContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
