...
try {
    ...
    print_r($transmissioncontroller->data_get());
} catch (\Throwable $err) {
...
$client = EleringDashboardSDK::test([
    "entity" => ["ummgascontroller" => ["test01" => ["id" => "test01"]]],
]);

// Entity ops return the ENTITY (throws on error);
// call data_get() for the mock record.
$ummgascontroller = $client->UmmGasController()->load(["id" => "test01"]);
print_r($ummgascontroller->data_get());
```
...
$umm_gas_controller = $client->UmmGasController()->load(["id" => 1]);
```
