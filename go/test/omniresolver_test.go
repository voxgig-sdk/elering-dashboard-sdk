...
import (
	...
	omni "github.com/voxgig-sdk/elering-dashboard-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.EleringDashboardError); is {
				out := map[string]any{
					"name":    "EleringDashboardError",
...
func omniCtx(arg any, client *sdk.EleringDashboardSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.EleringDashboardSDK); is {
			client = live
