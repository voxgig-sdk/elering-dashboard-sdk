...
func TestUmmRssFeedControllerEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		ummRssFeedControllerRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.umm_rss_feed_controller")))
...
func umm_rss_feed_controllerBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("ELERING_DASHBOARD_TEST_UMM_RSS_FEED_CONTROLLER_ENTID")
	...
	idmapResolved := core.ToMapAny(env["ELERING_DASHBOARD_TEST_UMM_RSS_FEED_CONTROLLER_ENTID"])
