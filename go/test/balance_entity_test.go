...
func TestBalanceEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set ELERINGDASHBOARD_TEST_BALANCE_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func balanceBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("ELERINGDASHBOARD_TEST_BALANCE_ENTID")
	idmapOverridden := entidEnvRaw != "" && strings.HasPrefix(strings.TrimSpace(entidEnvRaw), "{")

	env := envOverride(map[string]any{
		"ELERINGDASHBOARD_TEST_BALANCE_ENTID": idmap,
		"ELERINGDASHBOARD_TEST_LIVE":      "FALSE",
		"ELERINGDASHBOARD_TEST_EXPLAIN":   "FALSE",
		"ELERINGDASHBOARD_APIKEY":         "NONE",
	})

	idmapResolved := core.ToMapAny(env["ELERINGDASHBOARD_TEST_BALANCE_ENTID"])
