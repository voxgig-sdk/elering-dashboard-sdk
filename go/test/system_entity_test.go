...
func TestSystemEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set ELERINGDASHBOARD_TEST_SYSTEM_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func systemBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("ELERINGDASHBOARD_TEST_SYSTEM_ENTID")
	...
	idmapResolved := core.ToMapAny(env["ELERINGDASHBOARD_TEST_SYSTEM_ENTID"])
