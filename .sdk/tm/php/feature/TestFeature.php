...
// EleringDashboard SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class EleringDashboardTestFeature extends EleringDashboardBaseFeature
{
    ...
    public function init(EleringDashboardContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (EleringDashboardContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = EleringDashboardParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (EleringDashboardContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(EleringDashboardContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = EleringDashboardParam::call($ctx, $k);
