...
local GasTradeEntity = {}
GasTradeEntity.__index = GasTradeEntity


function GasTradeEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GasTradeEntity)
  self._name = "gas_trade"
  self._client = client
...
function GasTradeEntity:get_name()
  return self._name
end


function GasTradeEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GasTradeEntity.new(self._client, opts)
end
...
function GasTradeEntity:mark_deleted()
  self._deleted = true
end


function GasTradeEntity:deleted()
  return true == self._deleted
end


function GasTradeEntity:data_set(args)
  if args ~= nil then
...
function GasTradeEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GasTradeEntity:match_set(args)
  if args ~= nil then
...
function GasTradeEntity:match_get()
...
function GasTradeEntity:stream(action, args, callopts)
...
---@param reqmatch GasTradeLoadMatch
---@param ctrl? table
---@return GasTrade
---@return string? err
function GasTradeEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function GasTradeEntity:_run_op(ctx, post_done)
  local utility = self._utility
