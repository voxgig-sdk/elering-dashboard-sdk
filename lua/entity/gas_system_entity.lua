...
local GasSystemEntity = {}
GasSystemEntity.__index = GasSystemEntity


function GasSystemEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GasSystemEntity)
  self._name = "gas_system"
  self._client = client
...
function GasSystemEntity:get_name()
  return self._name
end


function GasSystemEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GasSystemEntity.new(self._client, opts)
end
...
function GasSystemEntity:mark_deleted()
  self._deleted = true
end


function GasSystemEntity:deleted()
  return true == self._deleted
end


function GasSystemEntity:data_set(args)
  if args ~= nil then
...
function GasSystemEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GasSystemEntity:match_set(args)
  if args ~= nil then
...
function GasSystemEntity:match_get()
...
function GasSystemEntity:stream(action, args, callopts)
...
---@param reqmatch GasSystemLoadMatch
---@param ctrl? table
---@return GasSystem
---@return string? err
function GasSystemEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function GasSystemEntity:_run_op(ctx, post_done)
  local utility = self._utility
