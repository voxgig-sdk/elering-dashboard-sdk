...
local InterruptibleEntity = {}
InterruptibleEntity.__index = InterruptibleEntity


function InterruptibleEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, InterruptibleEntity)
  self._name = "interruptible"
  self._client = client
...
function InterruptibleEntity:get_name()
  return self._name
end


function InterruptibleEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return InterruptibleEntity.new(self._client, opts)
end
...
function InterruptibleEntity:mark_deleted()
  self._deleted = true
end


function InterruptibleEntity:deleted()
  return true == self._deleted
end


function InterruptibleEntity:data_set(args)
  if args ~= nil then
...
function InterruptibleEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function InterruptibleEntity:match_set(args)
  if args ~= nil then
...
function InterruptibleEntity:match_get()
...
function InterruptibleEntity:stream(action, args, callopts)
...
---@param reqmatch InterruptibleLoadMatch
---@param ctrl? table
---@return Interruptible
---@return string? err
function InterruptibleEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function InterruptibleEntity:_run_op(ctx, post_done)
  local utility = self._utility
