-- EleringDashboard SDK InterruptibleCapacityController entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local InterruptibleCapacityControllerEntity = {}
InterruptibleCapacityControllerEntity.__index = InterruptibleCapacityControllerEntity


function InterruptibleCapacityControllerEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, InterruptibleCapacityControllerEntity)
  self._name = "interruptible_capacity_controller"
  self._client = client
...
function InterruptibleCapacityControllerEntity:get_name()
  return self._name
end


function InterruptibleCapacityControllerEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return InterruptibleCapacityControllerEntity.new(self._client, opts)
end
...
function InterruptibleCapacityControllerEntity:mark_deleted()
  self._deleted = true
end


function InterruptibleCapacityControllerEntity:deleted()
  return true == self._deleted
end


function InterruptibleCapacityControllerEntity:data_set(args)
  if args ~= nil then
...
function InterruptibleCapacityControllerEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function InterruptibleCapacityControllerEntity:match_set(args)
  if args ~= nil then
...
function InterruptibleCapacityControllerEntity:match_get()
...
function InterruptibleCapacityControllerEntity:stream(action, args, callopts)
...
---@param reqmatch InterruptibleCapacityControllerLoadMatch
---@param ctrl? table
---@return InterruptibleCapacityController
---@return string? err
function InterruptibleCapacityControllerEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function InterruptibleCapacityControllerEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return InterruptibleCapacityControllerEntity
