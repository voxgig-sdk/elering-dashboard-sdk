...
local RenominationEntity = {}
RenominationEntity.__index = RenominationEntity


function RenominationEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RenominationEntity)
  self._name = "renomination"
  self._client = client
...
function RenominationEntity:get_name()
  return self._name
end


function RenominationEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RenominationEntity.new(self._client, opts)
end
...
function RenominationEntity:mark_deleted()
  self._deleted = true
end


function RenominationEntity:deleted()
  return true == self._deleted
end


function RenominationEntity:data_set(args)
  if args ~= nil then
...
function RenominationEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RenominationEntity:match_set(args)
  if args ~= nil then
...
function RenominationEntity:match_get()
...
function RenominationEntity:stream(action, args, callopts)
...
function RenominationEntity:_run_op(ctx, post_done)
  local utility = self._utility
