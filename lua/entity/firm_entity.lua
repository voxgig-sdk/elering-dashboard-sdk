...
local FirmEntity = {}
FirmEntity.__index = FirmEntity


function FirmEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, FirmEntity)
  self._name = "firm"
  self._client = client
...
function FirmEntity:get_name()
  return self._name
end


function FirmEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return FirmEntity.new(self._client, opts)
end
...
function FirmEntity:mark_deleted()
  self._deleted = true
end


function FirmEntity:deleted()
  return true == self._deleted
end


function FirmEntity:data_set(args)
  if args ~= nil then
...
function FirmEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function FirmEntity:match_set(args)
  if args ~= nil then
...
function FirmEntity:stream(action, args, callopts)
...
---@param reqmatch FirmLoadMatch
---@param ctrl? table
---@return Firm
---@return string? err
function FirmEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function FirmEntity:_run_op(ctx, post_done)
  local utility = self._utility
