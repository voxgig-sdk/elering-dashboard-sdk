...
The Lua SDK for the EleringDashboard API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Balance(nil):load({ id = "example_id" }, nil)
...
local result, err = client:EleringDashboard(nil):load(
  { id = "test01" }, nil
)
