...
[MCP](https://modelcontextprotocol.io) server exposing the EleringDashboard SDK as
two agent tools — `elering-dashboard_list` and `elering-dashboard_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/elering-dashboard-mcp)
make build

# 2. Provide credentials via the environment
export ELERING_DASHBOARD_APIKEY=sk_live_xxx
...
```jsonc
// elering-dashboard_list: first page of records
{ "entity": "balance" }
{ "entity": "balance", "query": { } }

// elering-dashboard_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/elering-dashboard-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user elering-dashboard \
     -- "$PWD"/dist/*/elering-dashboard-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `elering-dashboard_list` and `elering-dashboard_load` tools now appear
   in new sessions. Ask the agent to *"list balance using elering-dashboard"*
   and it calls `elering-dashboard_list` with `{"entity":"balance"}`.
...
```sh
export ELERING_DASHBOARD_APIKEY=sk_live_xxx            # API key
export ELERING_DASHBOARD_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `elering-dashboard_list` tool
...
### Call the `elering-dashboard_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `elering-dashboard_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `elering-dashboard_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `ELERING_DASHBOARD_APIKEY` | API key sent with every request. |
| `ELERING_DASHBOARD_BASE` | Optional override of the API base URL. |
