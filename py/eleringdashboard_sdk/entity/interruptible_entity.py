...
class InterruptibleEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return InterruptibleEntity(self._client, opts)
    ...
    def data_get(self) -> Interruptible:
    ...
    def match_get(self) -> Interruptible:
    ...
    def load(self, reqmatch=None, ctrl=None) -> Interruptible:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Interruptible().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
