...
class RenominationEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return RenominationEntity(self._client, opts)
    ...
    def data_get(self) -> Renomination:
    ...
    def match_get(self) -> Renomination:
    ...
    def load(self, reqmatch=None, ctrl=None) -> Renomination:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Renomination().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
