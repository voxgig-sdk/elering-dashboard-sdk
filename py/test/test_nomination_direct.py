...
class TestNominationDirect:

    def test_should_direct_load_nomination(self):
        setup = _nomination_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-nomination", "live" if setup["live"] else "unit")
        if _skip:
...
def _nomination_direct_setup(mockres):
