...
class TestTransmissionControllerDirect:

    def test_should_direct_load_transmission_controller(self):
        setup = _transmission_controller_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-transmission_controller", "live" if setup["live"] else "unit")
        if _skip:
...
def _transmission_controller_direct_setup(mockres):
