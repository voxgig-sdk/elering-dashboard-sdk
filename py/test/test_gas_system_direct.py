...
class TestGasSystemDirect:

    def test_should_direct_load_gas_system(self):
        setup = _gas_system_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-gas_system", "live" if setup["live"] else "unit")
        if _skip:
...
def _gas_system_direct_setup(mockres):
