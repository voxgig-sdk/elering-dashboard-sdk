...
class TestNpsControllerDirect:

    def test_should_direct_load_nps_controller(self):
        setup = _nps_controller_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-nps_controller", "live" if setup["live"] else "unit")
        if _skip:
...
def _nps_controller_direct_setup(mockres):
