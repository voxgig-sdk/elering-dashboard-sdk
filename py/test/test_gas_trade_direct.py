...
class TestGasTradeDirect:

    def test_should_direct_load_gas_trade(self):
        setup = _gas_trade_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-gas_trade", "live" if setup["live"] else "unit")
        if _skip:
...
def _gas_trade_direct_setup(mockres):
