# EleringDashboard SDK test runner
...
class EleringDashboardTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            EleringDashboardTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = EleringDashboardTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = EleringDashboardTestRunner.getenv("ELERING_DASHBOARD_TEST_LIVE")
        override = EleringDashboardTestRunner.getenv("ELERING_DASHBOARD_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = EleringDashboardTestRunner.getenv(key)
        ...
        explain = EleringDashboardTestRunner.getenv("ELERING_DASHBOARD_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if EleringDashboardTestRunner._test_control is not None:
            return EleringDashboardTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                EleringDashboardTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            EleringDashboardTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = EleringDashboardTestRunner.load_test_control()
...
def load_env_local():
    EleringDashboardTestRunner.load_env_local()


def env_override(m):
    return EleringDashboardTestRunner.env_override(m)


def entity_data(v):
    return EleringDashboardTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return EleringDashboardTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return EleringDashboardTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return EleringDashboardTestRunner.load_test_control()


def live_client_options():
    return EleringDashboardTestRunner.live_client_options()


def live_delay_ms():
    return EleringDashboardTestRunner.live_delay_ms()
