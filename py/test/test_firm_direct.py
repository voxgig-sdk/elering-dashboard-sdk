...
class TestFirmDirect:

    def test_should_direct_load_firm(self):
        setup = _firm_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-firm", "live" if setup["live"] else "unit")
        if _skip:
...
def _firm_direct_setup(mockres):
